...
const TERRAIN_COLLISION_GROUP: u32 = 0b0001;
const PLAYER_COLLIDER_GROUP: u32 = 0b0010;
const METERS_PER_PIXEL: f32 = 0.01;
...
fn init(mut commands: Commands, mut rapier_config: ResMut<RapierConfiguration>) {
    ...
    rapier_config.scale = 1.0 / METERS_PER_PIXEL;
    rapier_config.gravity = Vec2::new(0.0, -10.0).into();
}
...
fn spawn_player(mut commands: Commands, asset_server: Res<AssetServer>) {
    commands
        ...
        .insert_bundle(RigidBodyBundle {
            position: Vec2::new(150.0 * METERS_PER_PIXEL, 230.0 * METERS_PER_PIXEL).into(),
        ...
        .insert_bundle(ColliderBundle {
            shape: ColliderShapeComponent(ColliderShape::ball(25.0 * METERS_PER_PIXEL)),
            material: ColliderMaterialComponent(ColliderMaterial {
                restitution: 0.0,
                ..Default::default()
            }),
            flags: ColliderFlags {
                collision_groups: InteractionGroups::new(PLAYER_COLLIDER_GROUP, u32::MAX),
                ..Default::default()
            }
            .into(),
            ..Default::default()
...
fn copy_physics_coordinates(mut positions: Query<(&RigidBodyPositionComponent, &mut Transform)>) {
    for (phys_pos, mut bevy_pos) in positions.iter_mut() {
        bevy_pos.translation.x = phys_pos.0.position.translation.x / METERS_PER_PIXEL;
        bevy_pos.translation.y = phys_pos.0.position.translation.y / METERS_PER_PIXEL;
    }
}

fn player_move(
    mut player: Query<&mut RigidBodyForcesComponent, With<Player>>,
    keys: Res<Input<KeyCode>>,
) {
    let mut player_forces = player.single_mut();
    let force_scale = 1000.0 * METERS_PER_PIXEL;

    if keys.pressed(KeyCode::W) {
        player_forces.force = Vec2::new(0.0, force_scale).into();
    }
    if keys.pressed(KeyCode::A) {
        player_forces.force = Vec2::new(-force_scale, 0.0).into();
    }
...
// fn cameraman(camera: Query<&mut Transform, With<Camera>>, player: Query<&Transform, With<Player>>)